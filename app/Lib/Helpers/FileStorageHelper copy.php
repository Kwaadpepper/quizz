...
/**
 * This class is used to handle files in LDC Laravel stack
 *
 ...
 *
 * @author Munsch Jérémy <jeremy.munsch@ldc.fr>
 */
class FileStorageHelper
{
    ...
    public static function storeFile(
        ...
        if (\is_object($file) and $file instanceof \Illuminate\Http\UploadedFile) {
            if ($private) {
                throw new \RuntimeException('Private storage for uploaded files is unhandled');
            }
    ...
    public static function storeModelFilesFromHtml(
        ...
        \collect(self::onlyUploadedFilesWithFileManager(self::findFilesInHtml($htmlString)))
            ->each(function (string $path) use ($model, &$htmlString) {
                ...
                $disk       = Storage::disk('publicUserLib');
                $fileInfo   = new \SplFileInfo($disk->path(
                    ToolboxHelper::mbReplace('/storage/_local/', '', \urldecode($path))
                ));
    ...
    public static function onlyUploadedFilesWithFileStorageHelper(array $urls): array
    {
        return \collect($urls)->unique()
            // * Filter to get only /_local and using app url or empty
        ...
        return \collect($urls)->unique()
            ...
            ->filter(function (string $url) {
                ...
                return ($host === \parse_url(config('app.url'), PHP_URL_HOST) or \is_null($host)) and
                    \strpos(\ltrim($path, '/'), 'storage/_local') === 0;
            })->filter(function (string $url) {
                $path = \parse_url($url, \PHP_URL_PATH);
                return Storage::disk('publicUserLib')
                    ->exists(ToolboxHelper::mbReplace('storage/_local/', '', \ltrim(\urldecode($path), '/')));
            })->all();
    }
