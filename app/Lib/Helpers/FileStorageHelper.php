...
class FileStorageHelper
{
    ...
    public static function removeOldFile(Model $model, string $attribute, bool $onlyIfUpdated = true): void
    {
        ...
        if ($onlyIfUpdated and $model->{$attribute} === ToolboxHelper::getOriginal($model, $attribute)) {
            return;
        }
        $originalPath = \ltrim(\urldecode(ToolboxHelper::getOriginal($model, $attribute)), '/');
    ...
    private static function getFileUniqueName(string $folderPath, string $filename): string
    {
        ...
        while (File::exists("{$folderPath}/{$filename}")) {
            if ($attempts++ >= $maxAttempts) {
                throw new \RuntimeException("Failed to have a unique filename for {$folderPath}/{$filename}");
            }
