...
import { ICardPosition, ICardProps, ICardStates, ICardTheme } from "./ICard";
...
export default class Card extends React.Component<ICardProps, ICardStates> {
    ...
    public render(): React.ReactElement {
        ...
        const { theme, style, classNames, collapsable, children, headerContent, position } = this.props;
        ...
        return (
            <div className={styleString}>
                <div className={styles.cardHeader}>
                    {headerContent && (
                        <h3 className={[styles.cardTitle, classNames?.title].join(" ")} style={style?.title}>
                            {headerContent}
                        </h3>
                    )}
                    {collapsable ?? false ? (
                        <button
                            className={[
                                styles.cardCloseButton,
                                (theme ?? ICardTheme.Dark) == ICardTheme.Light ? styles.cardButtonLight : "",
                                classNames?.closingButton,
                            ].join(" ")}
                            type="button"
                            onClick={this.collapse}
                            style={style?.closingButton}
                        >
                            {collapsed ? "+" : "-"}
                        </button>
                    ) : (
                ...
                {!collapsed && (headerContent != "" || (collapsable ?? false)) ? (
                    <hr
                        className={[
                            ...
                            (theme ?? ICardTheme.Dark) == ICardTheme.Light ? styles.separatorLight : "",
